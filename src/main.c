...
int alloc_objects(Scene *scene) {
    ...
    // Objects
    GameObject *objects = malloc(4 * sizeof(GameObject));
    if (objects == NULL) {
      return -1;
    }
...
int main() {
    ...
    for (int i = 0; i < frame_count; ++i) {
        ...
        Mat4x4 cam_matrix = transform_to_mat(scene.camera.transform);
        Mat4x4 view_matrix = mat_invert(&cam_matrix);
