...
void *load_fbx_scene(const char *model_path, const char *output_name) {
    ...
    auto upper_model_path_view =
        std::string(output_name) | std::views::transform(toupper);
    std::string upper_model_path(upper_model_path_view.begin(),
                                 upper_model_path_view.end());
    ...
    auto write_i = [&](int32_t i) { c_file << i << ", "; };
    for (size_t i = 0; i < scene->nodes.count; ++i) {
        ufbx_node *node = scene->nodes.data[i];
        if (node->is_root) continue;

        if (node->mesh) {
            auto sanitized = std::string(node->mesh->name.data) |
                             std::views::filter(isalnum) |
                             std::views::transform(tolower);
            std::string name(sanitized.begin(), sanitized.end());
            int32_t vert_count = node->mesh->vertices.count;
            h_file << "extern Vec3 " << name << "_verts[" << vert_count << "];"
                   << std::endl;
            ...
            h_file << "extern Vec3 " << name << "_norms[" << norm_count << "];"
                   << std::endl;
            ...
            h_file << "extern Vec2 " << name << "_uvs[" << uv_count << "];"
                   << std::endl;
            ...
            h_file << "extern Mesh " << name << "_mesh;" << std::endl;
            c_file << "Mesh " << name
                   << "_mesh = { "
                      ".ind_count = "
                   << vert_inds_count
