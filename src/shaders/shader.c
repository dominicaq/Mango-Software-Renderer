...
void vertex_shader(UBO *ubo, Vec4 a_position) {
    ...
    // Don't homogenize
    ubo->v_normal = vec3_normalize(vec4_to_vec3(mv_normal));
...
void fragment_shader(UBO *ubo, Vec3 frag_coord) {
    ...
    Vec3 N = vec3_normalize(ubo->gl_normal);
    ...
    lighting = vec3_scale(255.0f / ubo->num_lights,
                          lighting);  // this does not look right
